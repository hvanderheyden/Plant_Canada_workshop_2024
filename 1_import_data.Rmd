...
Before saving our work, lets inspect our phyloseq object, to make sure that everything is under control.

```{r}
sample_names(combinedPS_final)
rank_names(combinedPS_final)
sample_variables(combinedPS_final)
get_taxa(combinedPS_final)
```

Finally, we save our phyloseq object into the processed_data directory
```{r}
saveRDS(mock_raw, file= "Processed_data/combinedPS_final.rds")
```
